...
{
    var tottalMarks = 1100;
    var marksTaken = 928;
    const percentage = marksTaken / tottalMarks * 100;
...
var units = 278;
var billAmount = 12800;
if (units > 100) {
    const taxRate = 0.10; //10%tax = 0.10
    let taxAmount = billAmount * taxRate;
    let totalAmount = billAmount + taxAmount;
    console.log(totalAmount);
}
else if (units > 200) {
    const taxRate = 0.15; //15%tax = 0.15
    let taxAmount = billAmount * taxRate;
    let totalAmount = billAmount + taxAmount;
    console.log(totalAmount);
}
else if (units > 500) {
    const taxRate = 0.25; //25%tax = 0.25
    let taxAmount = billAmount * taxRate;
    let totalAmount = billAmount + taxAmount;
    console.log(totalAmount);
}
...
//   - Write a program that calculates the discount for a product based on its price. If the price is above $100, apply a 10% discount; otherwise, apply a 5% discount.
{
    ...
    if (productPrice > 100) {
        const discount = 0.10; // 10% discount = 0.10
        let discountAmount = productPrice * discount;
        let tottalAmount = productPrice - discountAmount;
        console.log(tottalAmount);
    }
    else {
        const discount = 0.05; // 5% discount = 0.05
        let discountAmount = productPrice * discount;
        let tottalAmount = productPrice - discountAmount;
        console.log(tottalAmount);
    }
